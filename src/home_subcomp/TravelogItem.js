...
const TravelogItem = ({id, title, writer, profileImg, travelImg, date, ment})=> {
    ...
    return(
        <li className="TravelogItem">
            <div className="info">
                <div className="profile"><img src={`./assets/travelPhoto${profileImg}`} /></div>
            ...
            <div className="photo">
                    <div className="mainImg"><img src={`assets/travelPhoto${travelImg[0]}`} alt="여행사진" /></div>
                    <div className="smallImg">
                        {travelImg.slice(1).map((it)=>(
                            <p key={it}><img src={`./assets/travelPhoto${it}`} /></p>
                        ))}
                    </div>
            </div>
